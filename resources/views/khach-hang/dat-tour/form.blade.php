                ...
                <div class="book-card">
                    ...
                    <div class="book-card-body">
                            ...
                            <label class="pttt-label">
                                <input type="radio" name="phuong_thuc_thanh_toan" value="dat_coc" {{ old('phuong_thuc_thanh_toan')=='dat_coc'?'checked':'' }} onchange="toggleAppointment(false); tinh()">
                                <div class="pttt-card">
                                    <div class="pttt-icon" style="background:rgba(74, 222, 128, 0.1);color:#4ade80;font-size:1.3rem;">💸</div>
                                    <div>
                                        <div class="pttt-name">Thanh toán đặt cọc (30%)</div>
                                        <div class="pttt-desc">Thanh toán trước 30%, phần còn lại trả sau tour</div>
                                    </div>
                            ...
                            <label class="pttt-label">
                                <input type="radio" name="phuong_thuc_thanh_toan" value="chuyen_khoan" {{ old('phuong_thuc_thanh_toan','chuyen_khoan')=='chuyen_khoan'?'checked':'' }} onchange="toggleAppointment(false); tinh()">
                                <div class="pttt-card">
                                    <div class="pttt-icon" style="background:#dbeafe;font-size:1.3rem;">🏦</div>
                                    <div>
                                        <div class="pttt-name">Chuyển khoản ngân hàng (100%)</div>
                                        <div class="pttt-desc">Thanh toán toàn bộ qua VietQR / Chuyển khoản</div>
                                    </div>
                                    <div class="pttt-radio"><span></span></div>
                                </div>
                            </label>
                            ...
                            <label class="pttt-label">
                                <input type="radio" name="phuong_thuc_thanh_toan" value="tien_mat" {{ old('phuong_thuc_thanh_toan')=='tien_mat'?'checked':'' }} onchange="toggleAppointment(this.checked); tinh()">
                                <div class="pttt-card">
                                    <div class="pttt-icon" style="background:#fef3c7;color:#d97706;font-size:1.3rem;">💵</div>
                                    <div>
                                        <div class="pttt-name">Thanh toán tiền mặt</div>
                                        <div class="pttt-desc">Thanh toán trực tiếp tại văn phòng VietGo</div>
                                    </div>
                                    <div class="pttt-radio"><span></span></div>
                                </div>
                            </label>
                            ...
                            {{-- Form hẹn tiền mặt --}}
                            <div id="appointment-box" style="display:{{ old('phuong_thuc_thanh_toan')=='tien_mat'?'block':'none' }}; margin-top:.5rem; padding:1.5rem; background:rgba(255,255,255,0.02); border:1px dashed rgba(255,255,255,0.1); border-radius:14px; animation: slideDown .3s ease;">
                                <div style="font-size:.85rem; font-weight:700; color:#fff; margin-bottom:1rem; display:flex; align-items:center; gap:.5rem;">
                                    <i class="fa-solid fa-clock-rotate-left" style="color:var(--primary-accent);"></i> Đặt Lịch Hẹn Tại Văn Phòng
                                </div>
                                <div class="contact-grid">
                                    <div class="cfield">
                                        <span class="cfield-label">Khu vực</span>
                                        <select name="khu_vuc" id="khu_vuc" class="cfield-input" onchange="updateProvinces()">
                                    ...
                                    <div class="cfield">
                                        <span class="cfield-label">Trụ sở / Chi nhánh</span>
                                        <select name="dia_diem_hen" id="dia_diem_hen" class="cfield-input">
                                            <option value="">-- Chọn trụ sở --</option>
                                        </select>
                                    </div>
                                    <div class="cfield" style="grid-column: span 2;">
                                        <span class="cfield-label">Thời gian dự kiến đến</span>
                                        <input type="datetime-local" name="thoi_gian_hen" class="cfield-input" min="{{ date('Y-m-d\TH:i') }}">
                                    </div>
                                </div>
                            </div>
                    </div>
                ...
                <div class="summary-card">
                    ...
                    <div class="summary-body">
                        ...
                        <div class="price-total-box">
                            <div>
                                <div class="price-total-label" id="psum-total-label">Tổng thanh toán</div>
                                <div id="psum-note" style="font-size:.72rem;color:rgba(255,255,255,.65);">Đã bao gồm thuế & phí</div>
                            </div>
...
function tinh() {
    ...
    // Cập nhật hiển thị đặt cọc 30% nếu được chọn
    const pttt = document.querySelector('input[name="phuong_thuc_thanh_toan"]:checked')?.value;
    const labelEl = document.getElementById('psum-total-label');
    const totalEl = document.getElementById('psum-total');
    const noteEl = document.getElementById('psum-note');

    if (pttt === 'dat_coc') {
        const coc = Math.round(cuoi * 0.3);
        labelEl.textContent = 'Tiền đặt cọc (30%)';
        totalEl.textContent = fmt(coc);
        totalEl.style.color = '#4ade80';
        noteEl.innerHTML = 'Còn lại 70% (' + fmt(cuoi - coc) + ') trả sau';
    } else {
        labelEl.textContent = 'Tổng thanh toán';
        totalEl.textContent = fmt(cuoi);
        totalEl.style.color = '';
        noteEl.textContent = 'Đã bao gồm thuế & phí';
    }
}

tinh();
function toggleAppointment(show) {
    const box = document.getElementById('appointment-box');
    box.style.display = show ? 'block' : 'none';
}

function updateProvinces() {
    const region = document.getElementById('khu_vuc').value;
    const branchSelect = document.getElementById('dia_diem_hen');
    branchSelect.innerHTML = '<option value="">-- Chọn trụ sở --</option>';

    const branches = {
        'mien_bac': [
            'Hà Nội (Trụ sở chính - 123 Lê Duẩn)',
            'Hải Phòng (Chi nhánh Lạch Tray)',
            'Quảng Ninh (Văn phòng Hạ Long)'
        ],
        'mien_trung': [
            'Đà Nẵng (Trụ sở miền Trung - 456 Võ Nguyên Giáp)',
            'Huế (Văn phòng Cố Đô)',
            'Nha Trang (Chi nhánh Trần Phú)'
        ],
        'mien_nam': [
            'TP. Hồ Chí Minh (Trụ sở miền Nam - 789 Nguyễn Huệ)',
            'Cần Thơ (Văn phòng Tây Đô)',
            'Vũng Tàu (Chi nhánh Thùy Vân)'
        ]
    };
